...
x_max = np.max(x)+0.1
nrmse_list = []
virt_nodes_list = np.linspace(20, 50, 10)
for no_virtual_nodes in virt_nodes_list:
    no_virtual_nodes = int(no_virtual_nodes)
    ...
    nrmse_list.append(min(nrmse_test))
...
# Plot NRMSE vs. number of virtual nodes
plt.figure()
plt.plot(virt_nodes_list, nrmse_list)
plt.xlabel('Number of virtual nodes')
plt.ylabel('NRMSE')
plt.title('NRMSE vs. Number of Virtual Nodes')
plt.grid(True)
plt.savefig('nrmse_vs_virtual_nodes.png')
plt.show()
